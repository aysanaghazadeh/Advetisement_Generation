...
class Evaluation:
    ...
    def evaluate_whoops_LLM(args):
        ...
        results = {'acc@1': 0}
        fieldnames = ['id', 'acc@1']
        csv_file_path = os.path.join(args.result_path, f'whoops_caption_new_hard_{args.description_type}_{args.VLM}_description_{args.LLM}_version_2.csv')
        with open(csv_file_path, 'w', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
    ...
    def evaluate_action_reason_LLM(args):
        ...
        descriptions = pd.read_csv(os.path.join(args.data_path, 'train',
                                                f'simple_llava_description_test_set.csv'))
        for image_url in QAs:
            description = descriptions.loc[descriptions['ID'] == image_url]['description'].values[0]
