...
class Evaluation:
    ...
    def evaluate_whoops_llava(self, args):
        ...
        fieldnames = ['id', 'acc@1']
        # csv_file_path = os.path.join(args.result_path, ''.join(['action_reason_llava_', args.description_file]))
        csv_file_path = os.path.join(args.result_path, 'whoops_explanation_QA_llava.csv')
        with open(csv_file_path, 'w', newline='') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
        ...
        QA_file = os.path.join(args.data_path, 'train/whoops_explanation.json')
        ...
        for i in QAs:
            image = Image.open(os.path.join(args.data_path, 'whoops_images', f'{i}.jpg'))
